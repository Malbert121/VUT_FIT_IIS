...
export const postSignUp = async (registerData:RegisterData) => {
    try {
        const response = await axios.post<AuthResponse>(`${API_CONFIG.API_BASE}/Auth/register`, registerData);
...
export const postSignIn = async (username:string, password:string) => {
    try {
        const response = await axios.post<AuthResponse>(`${API_CONFIG.API_BASE}/Auth/login`, {
            username,
            password,
        });
        localStorage.setItem('token', response.data.token);
